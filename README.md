...
> **No API key?** Run [`examples/quickstart_ollama.py`](examples/quickstart_ollama.py) against a local [Ollama](https://ollama.com) server — the same `consensus()` call with no paid endpoint required.
...
- **AI-native teams** building beyond chat — consensus voting, iterative refinement, and ReAct tool loops are the building blocks for LLM behaviour without pulling in a full framework.
