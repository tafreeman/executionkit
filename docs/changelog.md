...
All notable changes to ExecutionKit are documented here.
...
- `RetryConfig.max_retries` now means "retries **after** the initial call", so `with_retry()` makes up to `1 + max_retries` attempts instead of capping total attempts at `max_retries` **(behavior change)**. This matches what the `RetryConfig` docstring and `docs/engine-internals.md` already promised, and makes the field agree with `structured()`'s own `max_retries`, which has always run `1 + max_retries` attempts. Practically: an existing `RetryConfig(max_retries=3)` now dispatches up to 4 calls rather than 3 — one more call, one more unit of cost, and one more backoff sleep in the worst case for a request that keeps failing. To keep the old behavior exactly, decrement the value (`max_retries=N` → `max_retries=N-1`); `max_retries=0` is unchanged and still means a single attempt with no retries. A `max_cost` `llm_calls` ceiling still caps total dispatched attempts, so budget-capped callers see the budget bite one attempt sooner rather than an extra call
